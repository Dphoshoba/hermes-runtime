...
def assess_readiness(
    ...
    Args:
        ...
        protected_untracked: List of untracked files that belong to the
            user and must not be consumed by EVOSIA.
    ...
    if not _is_git_repo(repo):
        return RepositoryReadiness(
            ...
            recommendations=("Initialize a git repository before using EVOSIA",),
        )
    ...
    if detached:
        ...
        recommendations.append("Check out a branch before running EVOSIA")
    ...
    if conflicts:
        ...
        recommendations.append("Resolve merge conflicts before running EVOSIA")

    if staged:
        reasons.append(f"{len(staged)} staged file(s) present")
        recommendations.append("Commit or unstage changes before running EVOSIA")
    ...
    if not has_source:
        ...
        recommendations.append("Commit source code before running EVOSIA")

    if has_source and file_count < 2:
        reasons.append(f"Only {file_count} committed file(s) — likely empty project")
        recommendations.append("Commit meaningful source before running EVOSIA")
    ...
    if untracked:
        ...
        if protected_in_untracked:
            ...
            recommendations.append(
                "Use a git worktree to isolate EVOSIA changes from user work"
            )
